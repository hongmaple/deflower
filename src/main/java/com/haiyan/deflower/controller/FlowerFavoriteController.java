...
public class FlowerFavoriteController {
    ...
    public AjaxResult verify(@PathVariable Long favoriteId) {
        AjaxResult ajaxResult = AjaxResult.success("验证是否收藏成功",flowerFavoriteService.verify(favoriteId,0));
        return ajaxResult;
    }
     ...
     */
    @ApiOperation("验证是否收藏")
    @GetMapping("/video/{favoriteId}")
    public AjaxResult verifyVideo(@PathVariable Long favoriteId) {
        AjaxResult ajaxResult = AjaxResult.success("验证是否收藏成功",flowerFavoriteService.verify(favoriteId,1));
        return ajaxResult;
    }
    ...
    @ApiOperation("加载所有收藏")
    @PostMapping("/list")
    public AjaxResult listCollection(@RequestBody CollectionQuery collectionQuery) {
        AjaxResult ajaxResult = AjaxResult.success("加载所有收藏",flowerFavoriteService.listCollection(collectionQuery));
        return ajaxResult;
    }
