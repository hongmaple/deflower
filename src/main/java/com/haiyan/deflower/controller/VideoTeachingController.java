...
public class VideoTeachingController {
    ...
    @PostMapping("/create")
    @ApiOperation("添加视频教学")
    public AjaxResult addCVideoTeaching(@RequestBody VideoTeaching videoTeaching) {
        videoTeaching.setCreatorType(1);
        Long id = this.videoTeachingService.addVideoTeaching(videoTeaching);
        AjaxResult ajaxResult = AjaxResult.success("添加视频教学成功",id);
        return ajaxResult;
    }
    ...
    public AjaxResult getVideoTeachingById(@PathVariable Long id) {
        AjaxResult ajaxResult = AjaxResult.success(videoTeachingService.getVideoTeachingById(id));
        return ajaxResult;
    }
}
